task :deploy => ['deploy:app', 'deploy:push', 'deploy:restart', 'deploy:tag']
 
namespace :deploy do
  task :migrations => [:app, :push, :off, :migrate, :restart, :on, :tag]
  task :rollback => [:app, :off, :push_previous, :restart, :on]
  
  task :app do
    puts 'heroku git:remote -a gltv'
  end
  ...
  task :push_previous do
    ...
    if previous_release
      ...
      puts `git tag #{previous_release}`
      puts `git push --tags heroku`
      
      puts "Turning local repo checked out on master ..."
      puts `git checkout master`

      puts "Deleting local branch '#{previous_release}'"
      puts `git branch -d #{previous_release}`
      puts 'All done!'
    else
